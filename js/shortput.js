...
$(document).ready(function () {
  ...
  // });
  fetchDataAndDisplay("#shortputTabDiv");

  function fetchDataAndDisplay(tabDiv) {
    $.ajax({
      url: "http://10.244.1.180:8080/sportCategory/8",
      method: "GET",
      dataType: "json",
  ...
  function displayCategories(categories, tabDiv) {
    ...
    $.each(categories, function (index, category) {
      ...
      label.addClass("lab" + (index + 4)).attr("id", "label" + (index + 4));
      ...
      success: function (response) {
        ...
        response.forEach(function (eventData, index) {
          ...
          var card = $("<div>").addClass(
            "card1 data-table-card col-md-12 shotputCardTable"
          );
          ...
          card.appendTo("#table-row-shotput");
          $("#table-row-shotput").addClass("highlight");
        });
      },
      error: function (xhr, status, error) {
