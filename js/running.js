...
$(document).ready(function () {
  ...
  // });
  fetchDataAndDisplay("#runningTabDiv");

  function fetchDataAndDisplay(tabDiv) {
    $.ajax({
      url: "http://10.244.1.180:8080/sportCategory/7",
      method: "GET",
      dataType: "json",
      ...
      success: function (response) {
        ...
        response.forEach(function (eventData, index) {
          ...
          var card = $("<div>").addClass(
            "card1 data-table-card col-md12 runningCardTable "
          );
          ...
          console.log("team name", eventData);
          // var teamName = eventData.team[0].team1;
          // var team2 = eventData.team2[0];
          var setsTeam1 = JSON.parse(eventData.points);
          // var setsTeam2 = JSON.parse(eventData.points)[1].sets;

          // Create table rows for team1 and team2
          var team1Row = $("<div>").appendTo(tableBody);
          // team1Row.append(
          //   $("<td>").html(
          //     "<h5>" +
          //       team1.partName +
          ...
          //   )
          // );
          // eventData.team1.forEach(function (teamMember) {
          ...
          //         "</h5>" +
          //         "<p class='teamName'>" +
          //         teamMember.teamMaster.teamName +
          //         "</p>"
          //     )
          //   );
          // });
          ...
          //   team1Row.append(newRow);
          // });
          // setsTeam1.forEach(function (setDetail) {
          //   team1Row.append($("<td>").text(setDetail.point));
          // });

          // Assuming team1Row is the table body where you want to append the rows
          // Assuming team1Row is the table body where you want to append the rows
          eventData.team1.forEach(function (teamMember, index) {
            // Create a new row for each teamMember
            var newRow = $("<tr>");

            // Append the name and team information in the first cell
            var nameCell = $("<td>").html(
              "<h5>" +
                teamMember.partName +
                "</h5>" +
            ...
            if (setsTeam1[index] && setsTeam1[index].point) {
              var pointCell = $("<td>").text(setsTeam1[index].point);
          ...
          card.appendTo("#table-row-ruuning");
          $("#table-row-ruuning").addClass("highlight");
        });
      },
      error: function (xhr, status, error) {
