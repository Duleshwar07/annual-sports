...
$(document).ready(function () {
  ...
  // });
  fetchDataAndDisplay("#longJumpTabDiv");

  function fetchDataAndDisplay(tabDiv) {
    $.ajax({
      url: "http://10.244.1.180:8080/sportCategory/6",
      method: "GET",
      dataType: "json",
  ...
  function displayCategories(categories, tabDiv) {
    ...
    $.each(categories, function (index, category) {
      ...
      label.addClass("lab" + (index + 6)).attr("id", "label" + (index + 6));
      ...
      success: function (response) {
        ...
        response.forEach(function (eventData, index) {
          ...
          var card = $("<div>").addClass(
            "card1 data-table-card col-md-12 longjumpCardTable"
          );
          ...
          card.appendTo("#table-row-longjump");
          $("#table-row-longjump").addClass("highlight");
        });
      },
      error: function (xhr, status, error) {
